...
def load(s3, s3def, args, loaders, results):
    ...
    return model, results, transforms_vision
...
def Test(args, s3, s3def, model, loaders, device, results, writer, transforms_vision, profile=None):
    torch.cuda.empty_cache()
    # now = datetime.now()
...
def main(args):
    ...
    classify, results, transforms_vision = load(s3, s3def, args, loaders, results)
    ...
    if args.test:
        if args.profile:
            with profile(
                ...
                results = Test(args, s3, s3def, classify, loaders, device, results, writer, transforms_vision, prof)
        else:
            results = Test(args, s3, s3def, classify, loaders, device, results, writer, transforms_vision)
